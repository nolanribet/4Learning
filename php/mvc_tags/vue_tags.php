<?php
/**
 * VUE: Composants de l'interface pour les Matières (Tags) 🏷️
 */
...
function html_tr_matiere($matiere) {
    ...
    $html .= "<td><div style='width:20px; height:20px; border-radius:50%; background-color:$color; border:1px solid #ccc;'></div></td>";
    $html .= "<td>";
...
/** Formulaire de modification (UPDATE) */
function html_form_maj_matiere($matiere) {
    $id    = $matiere["id"]; 
    $name  = htmlspecialchars($matiere["name"]); 
    $color = htmlspecialchars($matiere["color"]); 
    
    // FIX: Redirection vers admin_tags.php
    $html = "<form action='admin_tags.php' method='POST' class='form-container'>\n"; 
